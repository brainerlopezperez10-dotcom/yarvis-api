  ...
  for (const modeloActual of MODELOS_GRATUITOS) {
    ...
    for (let intento = 1; intento <= 2; intento++) {
      try {
        ...
        console.error(`[IA] Intento ${intento} con ${modeloActual} falló:`, error.message);
        ultimoError = error;
        conversaciones.delete(`${sessionId}_${modo}`);

        // Si es saturación (503), esperamos 1.5 segundos antes del siguiente intento
        if (error.message.includes('503') || error.message.includes('high demand')) {
          await esperar(1500);
        } else {
          break; // Si no es error de demanda/saturación, probamos el siguiente modelo
        }
      }
    }
  }

  throw new Error(`Los servidores de Google están bajo alta demanda temporal. Intenta tu mensaje de nuevo en unos segundos. (Detalle: ${ultimoError?.message})`);
}
