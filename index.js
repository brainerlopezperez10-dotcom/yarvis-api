...
const SYSTEM_INSTRUCTION = `Tu nombre es ${NOMBRE_IA}. Eres un asistente virtual inteligente, atento y amigable. Tienes capacidad de ver fotografías, escuchar dictados y hablar. Responde de forma clara y útil.`;
...
// 1. INTERFAZ WEB CON SÍNTESIS DE VOZ (TEXT-TO-SPEECH)
app.get('/', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html lang="es">
    <head>
      ...
      <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: system-ui, -apple-system, sans-serif; }
        ...
        header { background: #1e293b; padding: 15px; text-align: center; border-bottom: 1px solid #334155; display: flex; justify-content: space-between; align-items: center; }
        header h1 { color: #38bdf8; font-size: 20px; }
        .btn-header { background: transparent; border: 1px solid #38bdf8; color: #38bdf8; padding: 6px 12px; border-radius: 6px; cursor: pointer; font-size: 14px; display: flex; align-items: center; gap: 6px; }
        .btn-header.active { background: #0284c7; color: white; border-color: #0284c7; }
        .chat-box { flex: 1; padding: 15px; overflow-y: auto; display: flex; flex-direction: column; gap: 10px; }
        .msg { max-width: 80%; padding: 10px 14px; border-radius: 12px; font-size: 14px; line-height: 1.4; word-break: break-word; position: relative; }
    ...
    <body>
      <header>
        ...
        <button id="btnAutoTTS" class="btn-header" onclick="toggleAutoTTS()">🔊 Voz Auto: OFF</button>
      </header>
      
      <div class="chat-box" id="chat">
        <div class="msg yarvis">
          <span>¡Hola! Soy ${NOMBRE_IA}. Ahora puedo leerte mis respuestas en voz alta. Presiona el botón 🔊 en la esquina de cada mensaje o activa la lectura automática en el encabezado.</span>
      ...
      <script>
        ...
        function toggleAutoTTS() {
          ...
          if (vozAutomatica) {
            ...
            btn.innerText = '🔊 Voz Auto: ON';
          } else {
            btn.classList.remove('active');
            btn.innerText = '🔊 Voz Auto: OFF';
        ...
        async function enviar() {
          ...
          try {
            ...
            // Construimos la burbuja del mensaje con botón de audio
            ...
            // Si está activada la lectura automática, se reproduce la respuesta
            if (vozAutomatica) {
              leerTexto(respuesta);
            }
          } catch(e) {
