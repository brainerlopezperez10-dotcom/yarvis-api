...
async function buscarEnInternet(pregunta) {
  ...
  // Tavily limita el campo "query" a 400 caracteres.
...
function formatearResultadosBusqueda(datos) {
  ...
  for (var i = 0; i < resultados.length; i++) {
    var r = resultados[i];
    // Recortamos el contenido de cada fuente para no inflar el payload que se le manda a Groq
...
async function procesarRespuestaIA(mensaje, sessionId, modo, imagen, mimeType, userId) {
  ...
  try {
    ...
    // El contexto de busqueda se manda solo en esta llamada puntual a Groq,
    // pero NO se guarda en el historial permanente de la sesion (evita que
    // el payload crezca sin control turno tras turno y provoque el 413).
