...
// MODELOS OFICIALES Y ESTABLES EN LA API DE GOOGLE
const MODELOS_GRATUITOS = [
  'gemini-2.0-flash',
  'gemini-1.5-flash'
];

async function obtenerChatSesion(sessionId, modo = 'asistente', modeloNombre = 'gemini-2.0-flash') {
...
// PROCESAMIENTO CON FALLBACK SEGURO
async function procesarRespuestaIA(mensaje, sessionId, modo, imagen, mimeType) {
  ...
  for (const modeloActual of MODELOS_GRATUITOS) {
    try {
      ...
      console.error(`[IA] Error con modelo ${modeloActual}:`, error.message);
      ultimoError = error;
      
      // Limpia la sesión actual para no reusar el modelo con fallos en el siguiente intento
