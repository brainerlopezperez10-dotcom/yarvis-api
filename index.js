...
// INSTRUCCIONES SEGÚN EL MODO SELECCIONADO
const PROMPTS_MODO = {
...
async function obtenerModeloValido() {
  ...
  for (const nombreModelo of MODELOS_GRATUITOS) {
    try {
      ...
      if (res.text()) {
        console.log(`✅ Modelo verificado: ${nombreModelo}`);
...
async function obtenerChatSesion(sessionId, modo = 'asistente') {
  ...
  // Si cambia el modo o no existe la sesión, creamos una nueva configuración
...
// INTERFAZ WEB CON SELECTOR DE MODOS
app.get('/', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html lang="es">
    <head>
      ...
      <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: system-ui, -apple-system, sans-serif; }
        ...
        header h1 { color: #38bdf8; font-size: 18px; display: flex; align-items: center; gap: 8px; }
    ...
    <body>
      ...
      <div class="chat-box" id="chat">
        <div class="msg yarvis">
          <span>¡Hola! Soy ${NOMBRE_IA}. Selecciona el modo en la barra superior según lo que necesites hoy.</span>
...
app.post('/chat', async (req, res) => {
  try {
    ...
    if (error.message.includes('429')) {
      return res.status(429).json({ error: "Límite alcanzado. Espera un minuto." });
    }
...
if (telegramToken) {
  ...
  bot.on('message', async (msg) => {
    ...
    if (mensajeTexto === '/reset') {
      conversaciones.delete(`telegram_${chatId}_asistente`);
    ...
    try {
      ...
      if (msg.photo) {
        ...
        const model = genAI.getGenerativeModel({ 
          model: nombreModelo,
          systemInstruction: PROMPTS_MODO.asistente
        });
        ...
        try {
          const chat = await obtenerChatSesion(sessionKey, 'asistente');
          const result = await chat.sendMessage(mensajeTexto);
          const response = await result.response;
          bot.sendMessage(chatId, response.text());
        } catch (e) {
          conversaciones.delete(`${sessionKey}_asistente`);
          modeloActivoConfirmado = null;
          const model = genAI.getGenerativeModel({ model: nombreModelo });
      ...
      bot.sendMessage(chatId, "Inconveniente al procesar la solicitud.");
    }
  });
