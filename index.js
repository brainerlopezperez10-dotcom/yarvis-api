...
// Servir interfaz gráfica desde /public
...
// Prompts para los diferentes modos de Yarvis
const PROMPTS_MODO = {
...
// MODELOS DE GROQ (Llama 3.3 para texto y Llama 3.2 Vision para fotos)
const MODELO_TEXTO = 'llama-3.3-70b-versatile';
const MODELO_VISION = 'llama-3.2-11b-vision-preview';

/**
 * Obtiene o inicializa el historial de una sesión de chat
 */
...
/**
 * Procesa la respuesta enviando la conversación a Groq
 */
async function procesarRespuestaIA(mensaje, sessionId, modo, imagen, mimeType) {
  ...
  try {
    ...
    if (imagen && mimeType) {
      ...
      // Para fotos enviamos el prompt del sistema + la imagen en formato Data URI
      const promptSistema = PROMPTS_MODO[modo] || PROMPTS_MODO.asistente;
      mensajesPayload = [
        { role: 'system', content: promptSistema },
        {
    ...
    // Si fue solo texto, guardamos la respuesta de la IA en el historial
    if (!imagen) {
...
if (telegramToken) {
  ...
  bot.on('message', async (msg) => {
    ...
    if (mensajeTexto === '/start') {
      return bot.sendMessage(chatId, `¡Hola! Soy ${NOMBRE_IA} impulsado por Groq. Puedes enviarme textos o imágenes.`);
    }
...
app.listen(PORT, () => console.log(`Servidor de ${NOMBRE_IA} (Groq) iniciado en el puerto ${PORT}`));
