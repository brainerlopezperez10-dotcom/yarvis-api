...
const AGENTES = {
  yarvis: {
    nombre: 'Yarvis',
    modelo: MODELO_TEXTO,
    prompt: `Tu nombre es Yarvis.
Eres el asistente principal, rápido, inteligente, útil y amigable.
Responde en español salvo que el usuario pida otro idioma.
Sé claro, directo y útil. No inventes información.`
  },

  atlas: {
    nombre: 'Atlas',
    modelo: MODELO_BUSQUEDA,
    prompt: `Tu nombre es Atlas.
Eres un investigador experto.
Analizas cuidadosamente las preguntas, distingues hechos de opiniones
y usas información actualizada cuando está disponible.
Resume primero la respuesta principal y después los detalles importantes.
Siempre menciona las fuentes cuando uses búsqueda.`
  },

  code: {
    nombre: 'Code',
    modelo: MODELO_TEXTO,
    prompt: `Tu nombre es Code.
Eres un programador experto.
Puedes ayudar con JavaScript, Node.js, HTML, CSS, Python, APIs, Express,
...
4. Evita complicaciones innecesarias.`
  ...
  nova: {
    nombre: 'Nova',
    modelo: MODELO_TEXTO,
    prompt: `Tu nombre es Nova.
Eres un asistente creativo.
Ayudas a crear historias, nombres, diseños, ideas, proyectos, guiones,
publicaciones, conceptos y textos persuasivos.
Sé original, inspirador y práctico.`
  },

  lumen: {
    nombre: 'Lumen',
    modelo: MODELO_TEXTO,
    prompt: `Tu nombre es Lumen.
Eres un tutor experto.
Explicas cualquier tema paso a paso, con ejemplos sencillos y claros.
Responde en español salvo que el usuario pida otro idioma.
Tu objetivo es que la persona realmente entienda.`
  }
};
...
function limpiarComando(mensaje) {
  ...
  return mensaje
    .replace(/^\/(atlas|code|nova|lumen|yarvis|investigar|programar|crear|idea|explicar)\s*/i, '')
    .trim();
}
...
function recortarHistorial(sesion) {
  ...
  if (resto.length > MAX_HISTORIAL) {
    sesion.mensajes = [system, ...resto.slice(-MAX_HISTORIAL)];
  }
}
...
function contextoMemoria(userId) {
  ...
  return `
MEMORIA DEL USUARIO:
${lista.slice(-20).map(item => `- ${item.text}`).join('\n')}
...
app.delete('/memory/:id', limitador, (req, res) => {
  const userId = String(req.query.userId || 'default');
  const id = req.params.id;
  const lista = obtenerMemorias(userId);

  memoria[userId] = lista.filter(item => item.id !== id);
  guardarJSON(MEMORY_FILE, memoria);
...
app.get('/conversations', limitador, (req, res) => {
  const userId = String(req.query.userId || 'default');
...
function guardarConversacion(userId, sessionId, titulo) {
  ...
  let conversacion = conversaciones[idUsuario].find(item => item.id === sessionId);
...
function formatearResultadosBusqueda(datos) {
  ...
  return datos.results.slice(0, 5).map((resultado, indice) => `
FUENTE ${indice + 1}:
...
async function procesarRespuestaIA(mensaje, sessionId, modo = 'asistente', imagen = null, mimeType = null, userId = 'default') {
  ...
  try {
    ...
    let mensajeParaIA = mensajeLimpio;
...
app.post('/chat', limitador, async (req, res) => {
  try {
    const {
      mensaje,
...
app.delete('/conversations/:sessionId', limitador, (req, res) => {
  const userId = String(req.query.userId || 'default');
  const sessionId = req.params.sessionId;

  if (conversaciones[userId]) {
    conversaciones[userId] = conversaciones[userId].filter(item => item.id !== sessionId);
    guardarJSON(CHATS_FILE, conversaciones);
  }
...
app.post('/reset', limitador, (req, res) => {
  const { sessionId = 'web_session', modo = 'asistente' } = req.body;
  const key = `\( {sessionId}_ \){modo}`;
  sesiones.delete(key);
...
app.post('/transcribe', limitador, async (req, res) => {
  try {
...
app.post('/tts', limitador, async (req, res) => {
  try {
...
app.get('*', (req, res) => {
  ...
  if (fs.existsSync(indexPath)) {
    ...
    res.json({ ia: NOMBRE_IA, mensaje: 'Yarvis está funcionando correctamente.' });
  }
});
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`🤖 ${NOMBRE_IA} está funcionando en el puerto ${PORT}`);
