...
function obtenerNombreModelo() {
  return MODELOS_DISPONIBLES[indiceModeloActual];
}

function rotarModelo() {
  indiceModeloActual = (indiceModeloActual + 1) % MODELOS_DISPONIBLES.length;
  console.log(`⚠️ Cambiando a modelo alternativo: ${obtenerNombreModelo()}`);
}

async function obtenerChatSesion(sessionId, modo = 'asistente') {
  const promptSistema = PROMPTS_MODO[modo] || PROMPTS_MODO.asistente;
  const keySesion = `${sessionId}_${modo}_${obtenerNombreModelo()}`;

  if (!conversaciones.has(keySesion)) {
    const model = genAI.getGenerativeModel({ 
      model: obtenerNombreModelo(),
...
// MANEJADOR PRINCIPAL DE CONSULTAS (CON RETRY AUTOMÁTICO)
async function procesarRespuestaIA(mensaje, sessionId, modo, imagen, mimeType) {
  let intentos = 0;
  const maxIntentos = MODELOS_DISPONIBLES.length;

  while (intentos < maxIntentos) {
    try {
      const modeloActual = obtenerNombreModelo();
      const promptSistema = PROMPTS_MODO[modo] || PROMPTS_MODO.asistente;
      ...
      console.error(`Error con modelo ${obtenerNombreModelo()}:`, error.message);

      // Si el error es 429 (Límite alcanzado), rotamos de modelo e intentamos de nuevo
      if (error.message.includes('429') || error.message.includes('quota') || error.message.includes('ResourceHasBeenExhausted')) {
        rotarModelo();
        intentos++;
      } else {
        throw error;
  ...
  throw new Error("Se superó el límite de peticiones en todos los modelos disponibles. Por favor espera unos segundos.");
}
...
if (telegramToken) {
  ...
  bot.on('message', async (msg) => {
    ...
    try {
      ...
      bot.sendMessage(chatId, "⚠️ El sistema está saturado. Por favor, espera 30 segundos antes de enviar otro mensaje.");
    }
  });
