...
// Únicamente modelos 100% gratuitos, públicos y estables
...
// Detectar automáticamente qué modelo gratuito está habilitado en tu API Key
async function obtenerModeloValido() {
  ...
  for (const nombreModelo of MODELOS_GRATUITOS) {
    try {
      ...
      console.log(`⚠️ Modelo ${nombreModelo} no disponible en esta API Key, probando siguiente...`);
    }
  }

  // Fallback por defecto en caso de no poder verificar previa petición
...
app.post('/chat', async (req, res) => {
  try {
    ...
    try {
      ...
      // Si ocurre cualquier fallo de memoria, reiniciamos la sesión y procesamos directo
